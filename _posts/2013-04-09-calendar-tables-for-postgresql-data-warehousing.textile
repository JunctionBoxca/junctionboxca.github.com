...
A personal goal of mine is to establish an apdex rating for all of the tiers in our infrastructure. An "apdex rating":http://apdex.org/ is a simple metric on a scale of 0.0 to 1.0 that avoids the skewing that can occur  when using averages (think big outliers e.g. 30s response for 1 connection out of hundreds).

To do this I'll be putting the apdex rating in the facts table and roll-up the score on an hourly basis into another table. The high-level schema for the data warehouse has the following dimensions;
...
Now that I'm almost past the fold in your browser lets discuss the calendar table.  The main use case for a calendar table is to trade-off space for performance by pre-populating and denormalising dates and time into unique fields within a table. It's pretty useful when asking questions like; "How much traffic did we have during Black Friday for the last 2 years?" By including apdex calculations you can ask "How did our infrastructure cope during that time?". 

As mentioned the schema for a calendar table is heavily denormalised around attributes of a date/time. The fields I feel are most useful in our current use case are;
...
Which yields a table DDL that looks a little like this before indexes are applied;
...
A small sample of data from the table looks as follows;
...
One thing I investigated before creating the DDL was URI length.  The "RFC":http://www.faqs.org/rfcs/rfc2616.html recommends limiting URI's to 255 bytes for (old) proxies. Well in a sample of 600k requests there were requests in excess of 4k long! They're likely an exploit but, still rather interesting.  Anyway that's it for today, time for bed!
