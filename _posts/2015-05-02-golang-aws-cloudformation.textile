...
---
Cloud computing has made getting an IT based company off the ground cheaper than it's ever been.However this ubiquity isn't without it's pain. As you develop your product inevitably you will increase the number of running services and their associated roles. Each service and role will have different requirements whether it be CPU, memory or IO requirements. As a result it requires that you have a heterogenus environment.  This heterogenity introduces complexity during the provisioning process which becomes a concern when reproducing an environment. Amazon CloudFormation is a technology that aims to reign in some of this complexity. At its core CloudFormation uses JSON based templates that produce stacks. CloudFormation manages all of the complexity of resource ordering for you (for AWS specific components) reducing the need for polling or subscribing to events. I've broken the remainder of this article in the following sections;
...
When first starting with CloudFormation templates there are 4 main elements you'll be concerned with;

- Parameters - variables that can be specified when you provision a "stack".
- Mappings - look-ups for various values.
- Resources - the composition of AWS services you want provisioned.
- Outputs - any variables or values you want to present to the user upon completion.
...
<pre>
go get -v github.com/awslabs/aws-sdk-go
</pre>
...
<pre>
PROJECT=$GOPATH/src/github.com/nfisher/goaws
mkdir -p "${PROJECT}"
...
This program will parse the template and either output the errors associated with it or print the templates description. Some readers may not the awsutil.StringValue and aws.String functions.  These are used distinguish between the absence of a value (nil pointer) and an empty value ("", 0, etc). You maybe used to this idiom if you've used protobufs.
...
<pre>
params := make([]*awscf.Parameter, len(template.Parameters)) //initialise an array to the length of the templates available parameters.
...
log.Println(awsutil.StringValue(cost.URL))
</pre>
...
This is the bit that everyone came for! Provisioning all the things at the speed of webscale! There's not a whole lot new here just building on what's already been done. The SDK meethod for provisioning a stack is;
...
<pre>
descInput := &awscf.DescribeStackEventsInput{
    StackName: aws.String(stackName),
  }
...
With a small dusting of code that is probably beginning to look as monotonous to read as it is to type;

<pre>
delInput := &awscf.DeleteStackInput{
    StackName: aws.String(stackName),
  }
...
log.Println(awsutil.StringValue(delOutput))
</pre>
