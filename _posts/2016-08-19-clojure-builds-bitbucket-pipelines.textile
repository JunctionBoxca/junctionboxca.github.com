...
Presumably you can skip the UI and get your builds running by simply adding a yaml file named bitbucket-pipelines.yml to the root of your repository. Either way once committed to the Bitbucket repository you should see your build happily kick off. My project is a Clojure/ClojureScript app and it took just over 3m to build. Not ideal but I certainly can't complain for a free offering. There's likely some caching I could do or possibly vendor all of the dependencies to reduce the build time. For now the service seems to work well and I look forward to exploring how the CD aspect works.
