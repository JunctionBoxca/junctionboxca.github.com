...
before_install:
  - test -f $HOME/.buck/build/successful-build || sudo apt-get update # only needed if buck is not built
  - test -f $HOME/.buck/build/successful-build || sudo apt-get install -y ant # only needed if buck is not built
  # Install Buck
...
With that config the build time will vary depending on whether it's a "clean" build or "incremental". My last "tweak":https://travis-ci.org/nfisher/cljbuck/builds/282049123 to .travis.yml resulted in a build that was done in 41s not too shabby!
...
4. If the build gets slow due to cache download process, purge it the next build will be a little pokey because it needs to build buck again but subsequent builds should be grease lightning.

Special thanks to "Markus Spiske":https://hackernoon.com/getting-started-with-buck-build-on-travis-ci-d1208d363023 for his article which was my starting point.
